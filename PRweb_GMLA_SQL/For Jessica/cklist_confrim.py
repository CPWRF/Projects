...
gmla_cklist = pd.read_sql(gmla_cklist_query, cnxn)
...
complete_rate['complete_rate'] = 100*(complete_rate.status / complete_rate.Project_Name).round(2)
...
on_time_rate['on_time_rate'] = (100* (on_time_rate.on_time_projects / on_time_rate.total_projects)).round(2)
on_time_rate['actual_on_time_rate'] = (100* ((on_time_rate.on_time_projects+ on_time_rate.delay_but_rejected) / on_time_rate.total_projects)).round(2)
...
with pd.ExcelWriter(getcwd()+'\data.xlsx', engine='xlsxwriter') as writer:
